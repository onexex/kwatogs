 ...
 *   - exportCash : Name + Net Pay list for cash release
 *   - exportCard : exact ATM bank-upload layout (grouped per company, running
 *                  number, name, account number, net amount, subtotals,
...
class PayrollExportController extends Controller
{
    ...
    private function netPay($row): float
    {
        return (float) ($row->net_pay ?? 0);
    }
    ...
    public function exportCard(Request $request)
    {
        ...
        $cashTotal = (float) $rows->reject(fn ($r) => $this->isCard($r))->sum(fn ($r) => $this->netPay($r));
        ...
        $x->setString('B3', 'NET PAYROLL FOR ATM', SimpleXlsx::S_TITLE);
    ...
    public function exportCash(Request $request)
    {
        ...
        $x->setString('A2', 'NET PAYROLL FOR CASH', SimpleXlsx::S_TITLE);
        ...
        $x->setString("C{$r}", 'NET PAY', SimpleXlsx::S_BOLD);
        $r++;
