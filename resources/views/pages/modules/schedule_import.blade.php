@extends('layout.app', ['title' => 'Schedule Import'])
@section('content')
...
<div class="ai-shell">
    <div class="ai-topbar">
        <div>
            <p class="page-title">Schedule Import</p>
            <p class="page-sub">Bulk-assign shifts over a date range — one row covers many days</p>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            <a href="{{ route('schedule-import.history') }}" class="btn-ghost">
                <i class="fa fa-clock-rotate-left"></i> Import History
            </a>
            <a href="{{ route('schedule-import.template') }}" class="btn-ghost">
    ...
    <div class="sc">
        <div class="sc-head">
            <div class="sc-icon"><i class="fa fa-calendar-plus"></i></div>
            <h5 class="sc-title">Upload File</h5>
        </div>
        <div class="sc-body">
            <div class="ai-note">
                <b>How it works:</b> one row = one employee + a <b>date range</b> + shift times. The system creates a
                schedule for <b>every day in the range</b>. To limit it to certain weekdays, fill the <b>Days</b> column with
                a comma list — <code>Mon,Tue,Wed,Thu,Fri</code> — or leave it blank for every day. Overnight shifts are
                detected automatically (when Schedule Out is earlier than Schedule In, the day rolls to the next).
                <b>Break is optional.</b> Employees are matched by Employee ID. A day that <b>overlaps an existing schedule</b>
                stops the whole import (nothing is saved) so you never double-book.
                Accepts <b>.xlsx</b> or <b>.csv</b>.
            </div>
    ...
    <div class="sc" id="aiResults" style="display:none;">
        ...
        <div class="sc-body">
            <div class="stat-grid">
                <div class="stat ok"><div class="n" id="rInserted">0</div><div class="l">Days Created</div></div>
...
$(function () {
    ...
    $('#aiForm').on('submit', function (e) {
        ...
        axios.post("{{ route('schedule-import.upload') }}", fd)
            .then(res => {
                Swal.close();
                const d = res.data;
                $('#rInserted').text(d.inserted); $('#rSkipped').text(d.skipped);
