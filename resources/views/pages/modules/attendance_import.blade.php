@extends('layout.app', ['title' => 'Attendance Import'])
@section('content')
...
<style>
    ...
    .ai-drop { border:2px dashed var(--border); border-radius:var(--radius-card); padding:30px; text-align:center;
        background:#fcfdfe; transition:all .15s; }
...
<div class="ai-shell">
    ...
    <div class="sc">
        <div class="sc-head">
            <div class="sc-icon"><i class="fa fa-file-import"></i></div>
            <h5 class="sc-title">Upload File</h5>
        </div>
        <div class="sc-body">
            <div class="ai-note">
                <b>How it works:</b> one row = one employee for one day. Each row creates/updates the
                <b>schedule</b>, the <b>home attendance</b> log, and the <b>daily summary</b>.
                Leave the summary minute columns blank to auto-compute from schedule vs actual times, or fill them to force values.
                Employees must already exist (matched by Employee ID). Re-importing the same employee + date updates instead of duplicating.
...
$(function () {
    ...
    $('#aiForm').on('submit', function (e) {
        ...
        axios.post("{{ route('attendance-import.upload') }}", fd)
            .then(res => {
                Swal.close();
                const d = res.data;
