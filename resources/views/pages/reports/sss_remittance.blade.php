@extends('layout.app', ['title' => 'SSS Contribution Report'])
@section('content')
...
<div class="rpt-shell">
    <div class="rpt-topbar">
        <p class="page-title"><i class="fa fa-building-columns me-2" style="color:var(--teal)"></i>SSS Contribution Report (R-3)</p>
        <p class="page-sub">Monthly SSS Contribution Collection List &mdash; employee &amp; employer shares per member, ready for R-3 filing.</p>
    </div>
        ...
        <div class="sc-body" style="padding:16px 22px;">
            <div class="row g-2 align-items-end">
                ...
                <div class="col-6 col-md-2">
                    ...
                    <input type="text" id="fltSearch" class="form-control form-control-sm" placeholder="Name / ID / SSS...">
                </div>
    ...
    <div class="sc">
        <div class="sc-head">
            <div class="sc-icon"><i class="fa fa-building-columns"></i></div>
            <h5 class="sc-title">SSS Collection List &mdash; <span id="lblPeriod">—</span></h5>
            <span class="pill ms-auto" id="lblEmployer">SSS Employer No.: —</span>
        </div>
        <div class="table-responsive" style="max-height:64vh; overflow:auto;">
            <table class="table table-hover align-middle mb-0 rpt-table">
                <thead>
                    <tr>
                        <th class="ps-3">SSS No.</th>
                        <th>Employee</th>
...
$(function () {
    ...
    function load() {
        ...
        $('#tfoot').html('');
        axios.get('/reports/sss/fetch', { params: params() }).then(res => {
            ...
            $('#lblEmployer').text('SSS Employer No.: ' + (d.employer_no || '—'));
            if (!rows.length) { $('#tbl').html('<tr><td colspan="6" class="text-center text-muted py-4">No SSS contributions found for this period.</td></tr>'); return; }
            let h = '';
            rows.forEach(r => {
                h += `<tr>
                    ...
                    <td class="text-end mono">${peso(r.ec)}</td>
                    <td class="text-end pe-3 fw-bold mono" style="color:var(--teal-dark)">${peso(r.total)}</td>
                </tr>`;
            });
            $('#tbl').html(h);
            $('#tfoot').html(`<tr><td colspan="2" class="text-end">TOTAL (${d.count})</td><td class="text-end mono">${peso(d.total_ee)}</td><td class="text-end mono">${peso(d.total_er)}</td><td class="text-end mono">${peso(0)}</td><td class="text-end pe-3 mono">${peso(d.total_all)}</td></tr>`);
    ...
    $('#btnExport').on('click', () => window.location = '/reports/sss/export?' + new URLSearchParams(params()).toString());
    $('#btnPrint').on('click', () => window.open('/reports/sss/print?' + new URLSearchParams(params()).toString(), '_blank'));
    load();
});
</script>
@endsection
